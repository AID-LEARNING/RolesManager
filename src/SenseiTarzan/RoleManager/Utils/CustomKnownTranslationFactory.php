...
class CustomKnownTranslationFactory
{
    ...
    public static function add_sub_roles_target(Role|array|string $data): Translatable
    {
        return new Translatable(CustomKnownTranslationKeys::ADD_SUB_ROLES_TARGET, ['subRoles' => self::format_roles_list_string($data)]);
    }

    public static function add_sub_roles_sender(Player|string $player, Role|array|string $data): Translatable
    {
        return new Translatable(CustomKnownTranslationKeys::ADD_SUB_ROLES_SENDER, ['player' => $player instanceof Player ? $player->getName() : $player, 'subRoles' => self::format_roles_list_string($data)]);
    }

    public static function remove_sub_roles_target(Role|array|string $data): Translatable
    {
        return new Translatable(CustomKnownTranslationKeys::REMOVE_SUB_ROLES_TARGET, ['subRoles' => self::format_roles_list_string($data)]);
    }

    public static function remove_sub_roles_sender(Player|string $player, Role|array|string $data): Translatable
    {
        return new Translatable(CustomKnownTranslationKeys::REMOVE_SUB_ROLES_SENDER, ['player' => $player instanceof Player ? $player->getName() : $player, 'subRoles' => self::format_roles_list_string($data)]);
    }

    public static function set_sub_roles_target(Role|array|string $data): Translatable
    {
        return new Translatable(CustomKnownTranslationKeys::SET_SUB_ROLES_TARGET, ['subRoles' => self::format_roles_list_string($data)]);
    }

    public static function set_sub_roles_sender(Player|string $player, Role|array|string $data): Translatable
    {
        return new Translatable(CustomKnownTranslationKeys::SET_SUB_ROLES_SENDER, ['player' => $player instanceof Player ? $player->getName() : $player, 'subRoles' => self::format_roles_list_string($data)]);
    }

    public static function role_not_found(Role|string $role): Translatable
    {
        return new Translatable(CustomKnownTranslationKeys::ROLE_NOT_FOUND, ['role' => $role instanceof Role ? $role->getName() : $role]);
    }

    private static function format_roles_list_string(Role|array|string $data): string
    {
        return is_array($data) ? implode(', ', array_map(function ($role) {
            return $role instanceof Role ? $role->getName() : $role;
        }, $data)) : ($data instanceof Role ? $data->getName() : $data);
    }
