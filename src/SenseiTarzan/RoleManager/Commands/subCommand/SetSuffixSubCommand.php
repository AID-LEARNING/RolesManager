...
class SetSuffixSubCommand extends BaseSubCommand
{
	...
	public function onRun(CommandSender $sender, string $aliasUsed, array $args) : void
	{
		...
		Await::g2c(RoleManager::getInstance()->setSuffix($target, $suffix), function (string $suffix) use ($sender, $target) {
			$sender->sendMessage(Main::getInstance()->getLanguageManager()->getTranslateWithTranslatable($sender, CustomKnownTranslationFactory::set_suffix_sender($target, $suffix)));
			$target->sendMessage(Main::getInstance()->getLanguageManager()->getTranslateWithTranslatable($target, CustomKnownTranslationFactory::set_suffix_target($suffix)));
		}, function () use ($sender, $target, $suffix) {
			$sender->sendMessage(Main::getInstance()->getLanguageManager()->getTranslateWithTranslatable($sender, CustomKnownTranslationFactory::error_set_suffix_sender($target, $suffix)));
		});
