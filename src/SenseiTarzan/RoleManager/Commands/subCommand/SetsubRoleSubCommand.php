...
class SetsubRoleSubCommand extends BaseSubCommand
{
	...
	public function onRun(CommandSender $sender, string $aliasUsed, array $args) : void
	{
		...
		Await::g2c(RoleManager::getInstance()->setSubRolesPlayer($target, $role), function (ResultUpdate $resultUpdate) use ($sender, $target) {
			$sender->sendMessage(Main::getInstance()->getLanguageManager()->getTranslateWithTranslatable($sender, CustomKnownTranslationFactory::set_sub_roles_sender($target, $role = $resultUpdate->data)));
			if ($resultUpdate->online) {
				$target->sendMessage(Main::getInstance()->getLanguageManager()->getTranslateWithTranslatable($target, CustomKnownTranslationFactory::set_sub_roles_target($role)));
			}
		}, function () {
